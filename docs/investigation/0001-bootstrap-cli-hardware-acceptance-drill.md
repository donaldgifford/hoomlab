...
| --- | --- |
| bootstrap CLI | built from `528976d` (`feat/impl-hardware-drill`, clean tree) via `just bootstrap-build` → `build/bin/bootstrap` (prior builds retired by fold-backs: `4b443ea`, `b7968c4`, `99b9416`, `f4ba8d5`, `797127e`, `6120bd1`) |
...
| 2026-08-25 | `pve certs`, `acme-plugin-cloudflare` re-run | Bonus find inside the flip attempt: the plugin step **rotated identical credentials** on a run where nothing changed — real PVE round-trips the stored payload in its own rendering, not the SDK's byte-exact encoding, so the byte-for-byte comparison never matched and the step would re-apply on every run (a permanent false-pending the convergence gate would have flunked). | Code fix `797127e`: the comparison is structural — decode the stored base64, parse `KEY=value` lines, compare maps — immune to ordering and whitespace normalization. Regression seeds a server-normalized payload and fails pre-fix. **Root cause landed one round later** (`528976d`): the live plugin index showed the stored payload at `len=62` — not a multiple of 4 — so PVE re-encodes as **unpadded base64**; the padded-only decode errored and the error was swallowed at debug level as "treat as drifted", so the structural comparison never ran. The decode now accepts both renderings; the undecodable path logs at Warn and value mismatches log key sets at Info — drift can never rotate silently again. |
