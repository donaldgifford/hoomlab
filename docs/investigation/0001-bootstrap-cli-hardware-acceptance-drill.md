...
| --- | --- |
| bootstrap CLI | built from `797127e` (`feat/impl-hardware-drill`, clean tree) via `just bootstrap-build` → `build/bin/bootstrap` (prior builds `4b443ea`, `b7968c4`, `99b9416`, `f4ba8d5` retired by fold-backs) |
...
| 2026-08-25 | `pve certs`, `acme-plugin-cloudflare` re-run | Bonus find inside the flip attempt: the plugin step **rotated identical credentials** on a run where nothing changed — real PVE round-trips the stored payload in its own rendering, not the SDK's byte-exact encoding, so the byte-for-byte comparison never matched and the step would re-apply on every run (a permanent false-pending the convergence gate would have flunked). | Code fix `797127e` (same commit): the comparison is structural — decode the stored base64, parse `KEY=value` lines, compare maps — immune to ordering and whitespace normalization. Regression seeds a server-normalized payload and fails pre-fix. |
